...
def upload_image():
  ...
  if input_image != None:
    ...
    genre = st.radio(
        "Choose Crop Algorithm : ",
        ('Choose line by its gradient', 'Choose line by how much intersection points in one line'))

    if genre == 'Choose line by its gradient':
        hough_image, drawed_img, drawed_img_lrtb, imgres, real = cr.crop_main(cvImg, 0)
    elif genre == 'Choose line by how much intersection points in one line':
